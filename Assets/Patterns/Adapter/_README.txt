Adapter is a structural design pattern that allows objects with incompatible interfaces to collaborate. The main difference between the facade and the adapter pattern is that the facade pattern does not change the interface of the system, but only simplifies it, while the adapter pattern changes the interface of the system to match another one.
